 ...
 *   5) 排行榜取得
 ********************************************/
const apiBaseUrl = "http://localhost:5000"; // 或使用雲端部署的後端 URL
...
async function getUserList() {
  ...
  const cloudData = await fetchUserList();
  const mergedData = mergeUserData(localData, cloudData); // 合併雲端與本地資料
  localStorage.setItem("userList", JSON.stringify(mergedData));
  return mergedData;
}

function setUserList(newList) {
  localStorage.setItem("userList", JSON.stringify(newList));
  newList.forEach(user => saveUserToCloud(user)); // 同步到雲端
}

// 合併資料
function mergeUserData(local, cloud) {
  const userMap = new Map();
  cloud.forEach(user => userMap.set(user.username, user));
  local.forEach(user => {
...
function setCurrentUser(updatedUser) {
  ...
  if (idx !== -1) {
    ...
    setUserList(userList); // 同步本地與雲端
    saveUserToCloud(updatedUser); // 單獨更新當前用戶到雲端
    }
}
